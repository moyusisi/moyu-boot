package com.moyu.boot.common.authZ.handler;
...
public class CustomAuthenticationEntryPoint implements AuthenticationEntryPoint {
    ...
    public void commence(HttpServletRequest request, HttpServletResponse response, AuthenticationException authException) throws IOException, ServletException {
        ...
        try {
            ...
            // 处理登录异常，区分未认证的具体场景
            result = ExceptionWrapperUtils.handleNotLogin(nle);
            // 记录日志
            String ip = ServletUtil.getClientIP(request);
            if (ObjectUtil.isNotEmpty(ip)) {
                log.info("Security Filter层，From Ip:{}, User-Agent:{}", ip, ServletUtil.getHeaderIgnoreCase(request, "User-Agent"));
            }
        }
        String responseBody = new ObjectMapper().writeValueAsString(result);
        log.info("Security Filter层，未认证访问{}，处理返回:{}", request.getRequestURI(), responseBody);
